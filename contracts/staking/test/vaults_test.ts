...
blockchainTests('Staking Vaults', env => {
    ...
    blockchainTests.resets('Reward Vault', () => {
        it.skip('basic management', async () => {
            ...
            await stakingWrapper.rewardVaultRegisterPoolAsync(
                poolId,
                poolOperator,
                operatorShare,
            ...
            let tx = stakingWrapper.rewardVaultRegisterPoolAsync(
                poolId,
                poolOperator,
                operatorShare,
                stakingContractAddress,
            ...
            tx = stakingWrapper.rewardVaultRegisterPoolAsync(
                poolId,
                poolOperator,
                operatorShare,
                notStakingContractAddress,
            );
            await expect(tx).to.revertWith(revertError);
        });
    });
